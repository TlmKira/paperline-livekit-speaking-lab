...
function DesktopAuthShell({
  ...
  children: ReactNode;
  copy: Awaited<ReturnType<typeof getI18n>>["dictionary"]["auth"];
}) {
  return (
...
export default async function AuthLayout({ children }: { children: ReactNode }) {
  ...
  if (runtime === "desktop") {
    return <DesktopAuthShell copy={dictionary.auth}>{children}</DesktopAuthShell>;
  }

  return <WebAuthShell copy={dictionary.auth}>{children}</WebAuthShell>;
}
